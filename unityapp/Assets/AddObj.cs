    ...
    public class AddObj : PressInputBase
    {
        ...
        async void AddPoster(string type, string user, string id)
        {
            ...
            if (m_RaycastManager.Raycast(new Vector2(Screen.width / 2, Screen.height / 2), s_Hits, TrackableType.PlaneEstimated))
            {
                ...
                if (type.Equals("posters") || type.Equals("stickers") || type.Equals("images"))
                {
                    ...
                    await storageRef.Root.Child("users/" + user + "/" + type + "/" + id + ".jpg").GetDownloadUrlAsync().ContinueWithOnMainThread(async task2 =>
                    {
                        if (task2.IsFaulted || task2.IsCanceled)
                        {
                            ...
                            // set diffuse texture
                            spawnedObject.GetComponent<MeshRenderer>().material.SetTexture("_img", texture);
                ...
                {
                    spawnedObject = Instantiate(m_PlacedPrefab, hitPose.position + hitPose.rotation * Vector3.up * 0.1f, transform.rotation * Quaternion.Euler(0, Camera.main.transform.rotation.eulerAngles.y, 0));
                }
            ...
            else
            {
                ...
                if (type.Equals("posters") || type.Equals("stickers") || type.Equals("images"))
                {
                    spawnedObject = Instantiate(m_PosterPrefab, Camera.main.transform.position + Camera.main.transform.forward * 2f, transform.rotation * Quaternion.Euler(0, Camera.main.transform.rotation.eulerAngles.y, 0));
                    // get poster image
                    StorageReference storageRef = FirebaseStorage.GetInstance(FirebaseApp.DefaultInstance).GetReferenceFromUrl("gs://ourworld-737cd.appspot.com");
                ...
                {
                    spawnedObject = Instantiate(m_PlacedPrefab, Camera.main.transform.position + Camera.main.transform.forward * 2f, transform.rotation * Quaternion.Euler(0, Camera.main.transform.rotation.eulerAngles.y, 0));
                }
        ...
        private void Update()
        {
            ...
            if (moveChild != null)
            {
                ...
                {
                    rotateChild.transform.position = spawnedObject.transform.position;
            ...
            else if (change.Equals("rotate"))
            {
                ...
                if (spawnedObject != null)
                {
                    ...
                    if (previousRotation != roundedRot)
                    {
                        ...
                        rotateChild.transform.rotation = Quaternion.Euler(roundedRot);
